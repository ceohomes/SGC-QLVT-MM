...
export default function DataTable({ rows, projects = [], onEdit, onDelete, onAddSubRow, pcuDays, currentUser, sortKey, sortDir, onSort }) {
  ...
  const getProjectInfo = (item) => {
    ...
    // Ưu tiên dùng trường duAn đã lưu sẵn trong DB
    if (item.duAn) {
      return { 
        label: item.duAn, 
        khoiTen: item.khoiTen || '—', 
        khoiVietTat: item.khoiVietTat || '' 
      }
    }
    ...
    const vt = p.khoiVietTat || p.vietTat || ''
    const kTen = p.khoiTen || p.ten || '—'
    const prefix = vt ? `${vt}. ` : ''
            ...
            return (
              <tr
                ...
                className={`group transition-colors hover:bg-royal-50/70
                  ${row.parentId 
                    ? 'bg-white italic text-slate-700' 
                    : (isEven ? 'bg-royal-50/30' : 'bg-royal-50/60')}
                  ${overdue ? 'outline outline-1 outline-rose-200 outline-offset-[-1px]' : ''}
                `}
              >
                {/* STT */}
                <td className="px-2 py-1.5 text-center text-slate-400 font-medium border-b border-r border-[#031240]/20 font-data text-[12px]">
                  {getStt()}
                </td>

                {/* Actions */}
                <td className="px-2 py-1.5 text-center border-b border-r border-[#031240]/20 whitespace-nowrap">
                  <div className="flex items-center justify-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                    {!row.parentId && (
                      <button
                        onClick={() => onAddSubRow(row)}
                        className="w-6 h-6 flex items-center justify-center bg-emerald-100 hover:bg-emerald-200 text-emerald-700 rounded-md transition-all shadow-sm"
                        title="Thêm dòng phụ"
                      >
                ...
                {/* Khối thi công */}
                <td className="px-2 py-1.5 text-center border-b border-r border-[#031240]/20 text-[12px]">
                  {info.khoiVietTat ? (
                ...
                {/* Dự án */}
                <td className="px-2 py-1.5 border-b border-r border-[#031240]/20 text-[12px]">
                  <div className="max-w-[170px] truncate flex items-center gap-1" title={info.label}>
                    {info.label !== 'Chưa phân bổ' && (
                      <span className="shrink-0 text-[10px] font-black px-1.5 py-0.5 rounded bg-slate-700 text-white truncate max-w-[170px]">
                        {info.label}
                      </span>
                    )}
                    {info.label === 'Chưa phân bổ' && <span className="text-slate-400 italic">Chưa phân bổ</span>}
                  </div>
                </td>

                {/* Mã vật tư */}
                <td className="px-2 py-1.5 border-b border-r border-[#031240]/20 whitespace-nowrap">
                ...
                {/* Tên vật tư */}
                <td className="px-2 py-1.5 font-medium text-slate-800 border-b border-r border-[#031240]/20">
                  <div className="max-w-[190px] truncate" title={row.tenVattu}>{row.tenVattu || '—'}</div>
                </td>

                {/* ĐVT */}
                <td className="px-2 py-1.5 text-center text-slate-500 border-b border-r border-[#031240]/20 whitespace-nowrap">
                  {row.dvt || '—'}
                </td>
                ...
                {/* Tên NCC */}
                <td className="px-2 py-1.5 text-slate-700 border-b border-r border-[#031240]/20">
                  <div className="max-w-[170px] truncate" title={row.tenNcc}>{row.tenNcc || '—'}</div>
                </td>

                {/* SL giao thực */}
                <td className="px-2 py-1.5 text-center font-semibold text-slate-700 border-b border-r border-[#031240]/20 font-data text-[12px]">
                  {row.soLuongGiaoThuc || '—'}
                </td>

                {/* Loại HĐ */}
                <td className="px-2 py-1.5 text-slate-600 border-b border-r border-[#031240]/20">
                  <div className="max-w-[130px] truncate text-[12px]" title={row.loaiHd}>{row.loaiHd || '—'}</div>
                </td>

                {/* Quy cách KT */}
                <td className="px-2 py-1.5 text-slate-500 border-b border-r border-[#031240]/20">
                  <div className="max-w-[190px] truncate text-[12px]" title={row.quyCachKyThuat}>{row.quyCachKyThuat || '—'}</div>
                </td>

                {/* Đợt */}
                <td className="px-2 py-1.5 text-center text-slate-600 border-b border-r border-[#031240]/20 whitespace-nowrap font-data text-[12px]">
                  {row.dot || '—'}
                </td>

                {/* Khối lượng */}
                <td className="px-2 py-1.5 text-center font-semibold text-slate-700 border-b border-r border-[#031240]/20 whitespace-nowrap font-data text-[12px]">
                  {row.khoiLuong || '—'}
                </td>
                ...
                {/* Ngày PCU trả */}
                <td className={`px-2 py-1.5 text-center border-b border-r border-[#031240]/20 whitespace-nowrap font-data text-[12px] ${overdue ? 'text-rose-600 font-bold' : 'text-slate-600'}`}>
                  {row.ngayPcuTra ? (
                ...
                {/* Ngày về DK bắt đầu */}
                <td className={`px-2 py-1.5 text-center border-b border-r border-[#031240]/20 whitespace-nowrap font-data text-[12px] ${!row.ngayVeDuKienBatDau ? 'text-rose-400' : 'text-slate-600'}`}>
                  {row.ngayVeDuKienBatDau || <span className="italic text-[11px]">Chưa nhập*</span>}
                </td>

                {/* Ngày về DK kết thúc */}
                <td className={`px-2 py-1.5 text-center border-b border-r border-[#031240]/20 whitespace-nowrap font-data text-[12px] ${!row.ngayVeDuKienKetThuc ? 'text-rose-400' : 'text-slate-600'}`}>
                ...
                {/* Ngày về TT */}
                <td className="px-2 py-1.5 text-center border-b border-r border-[#031240]/20 whitespace-nowrap font-data text-[12px]">
                  {row.ngayVeThucTe
                ...
                {/* KL còn thiếu */}
                <td className={`px-2 py-1.5 text-center font-bold border-b border-r border-[#031240]/20 whitespace-nowrap font-data text-[12px] ${
                ...
                {/* Ghi chú */}
                <td className="px-2 py-1.5 text-slate-500 border-b border-r border-[#031240]/20 last:border-r-0">
