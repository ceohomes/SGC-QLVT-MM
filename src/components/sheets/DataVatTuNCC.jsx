...
export default function DataVatTuNCC() {
  ...
  const handleImportVattu = async (e) => {
    ...
    try {
      ...
      const headerMap = {
        ...
        'Đơn vị tính': 'dvt', 'Loại vật tư': 'loaiVattu', 'Thông số kỹ thuật': 'thongSoKyThuat',
      }
  ...
  const handleExportVattu = async () => {
    const XLSX = await loadXLSX()
    const headers = [['Mã Vật Tư (Mã SAP)', 'Mã nhóm Vật tư', 'Tên nhóm Vật tư', 'Tên vật tư', 'Đơn vị tính', 'Loại vật tư', 'Thông số kỹ thuật']]
    const data = filteredVattu.map(i => [i.maVattuSap, i.maNhomVattu, i.tenNhomVattu, i.tenVattu, i.dvt, i.loaiVattu, i.thongSoKyThuat])
