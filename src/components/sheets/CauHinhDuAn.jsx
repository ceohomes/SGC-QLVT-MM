      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6 flex flex-col gap-4">
        ...
        <div className="flex flex-col gap-3">
          <div>
            ...
            <input autoFocus className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-400"
            ...
            <div className="flex gap-2 flex-wrap">
              {PALETTE.map((p, i) => (
                <button key={i} onClick={() => setPaletteIdx(i)}
                  className={`w-7 h-7 rounded-lg border-2 transition-all ${paletteIdx === i ? 'scale-125 ring-2 ring-offset-1 ring-blue-500' : 'hover:scale-110'}`}
        ...
        <div className="flex gap-2 pt-1">
          <button onClick={submit} className="flex-1 flex items-center justify-center gap-2 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg text-sm font-bold transition-all">
            <Check className="w-4 h-4" />{isEdit ? 'Lưu thay đổi' : 'Thêm khối'}
          </button>
          <button onClick={onClose} className="px-4 py-2 bg-slate-100 hover:bg-slate-200 text-slate-600 rounded-lg text-sm font-semibold transition-all">Hủy</button>
        </div>
      </div>
    </div>
  )
...
function DuAnRow({ duAn, badge, badgeColor, onDelete, onRename }) {
  ...
  return (
    <div
      className="flex items-center gap-2 px-3 py-2 bg-white rounded-xl border border-slate-100 shadow-sm hover:shadow-md hover:border-slate-200 transition-all group"
      onMouseEnter={() => setHover(true)} onMouseLeave={() => setHover(false)}
    >
      <GripVertical className="w-3.5 h-3.5 text-slate-300 shrink-0 group-hover:text-slate-400 cursor-grab" />
      <span className="text-[11px] font-black px-1.5 py-0.5 rounded-md text-white shrink-0 min-w-[32px] text-center" style={{ background: badgeColor }}>{badge}</span>
      {editing ? (
      ...
      {hover && !editing && (
        <div className="flex items-center gap-0.5 shrink-0">
          <button onClick={() => setEditing(true)} className="w-5 h-5 rounded flex items-center justify-center text-slate-300 hover:text-blue-500 transition-all"><Edit2 className="w-3 h-3" /></button>
          <button onClick={onDelete} className="w-5 h-5 rounded flex items-center justify-center text-slate-300 hover:text-rose-500 transition-all"><X className="w-3 h-3" /></button>
        </div>
      )}
    </div>
  )
...
function KhoiColumn({ khoi, searchQ, onDelete, onEdit, onAddDuAn, onDeleteDuAn, onRenameDuAn }) {
  ...
  return (
    <div className="flex flex-col rounded-2xl border-2 shrink-0 w-[260px] overflow-hidden shadow-sm" style={{ background: p.bg, borderColor: p.border, maxHeight: 'calc(100vh - 160px)' }}>
      {/* Header */}
      <div className="flex items-center gap-1.5 px-3 pt-3 pb-2 shrink-0">
        <span className="flex-1 text-[11px] font-black uppercase tracking-wider text-slate-700 truncate">{khoi.ten}</span>
        <span className="text-[10px] font-bold text-slate-400">{filtered.length}</span>
        <span className="text-[11px] font-black px-2 py-0.5 rounded-lg text-white" style={{ background: p.badge }}>{khoi.vietTat}</span>
        <button onClick={() => onEdit(khoi)} className="w-6 h-6 rounded flex items-center justify-center text-slate-400 hover:text-blue-600 hover:bg-white/60 transition-all"><Edit2 className="w-3 h-3" /></button>
        <button onClick={() => onDelete(khoi.id)} className="w-6 h-6 rounded flex items-center justify-center text-slate-400 hover:text-rose-500 hover:bg-white/60 transition-all"><Trash2 className="w-3 h-3" /></button>
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-2 py-2 space-y-1.5">
        ...
        {adding && (
          <div className="flex items-center gap-2 px-2 py-1.5 bg-white rounded-xl border-2 border-blue-400 shadow">
            ...
            <button onClick={submit} className="text-blue-500 hover:text-blue-700 shrink-0"><Check className="w-4 h-4" /></button>
            <button onClick={() => { setAdding(false); setNewTen('') }} className="text-slate-400 hover:text-slate-600 shrink-0"><X className="w-4 h-4" /></button>
          </div>
        )}
      </div>
      ...
      <div className="shrink-0 px-2 pb-2 pt-1">
        <button onClick={() => setAdding(true)}
          className="w-full flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-slate-500 hover:bg-white/70 hover:text-slate-700 text-xs font-semibold transition-all">
...
export default function CauHinhDuAn() {
  ...
  const [modalKhoi, setModalKhoi] = useState(null) // null=closed, 'new'=add, object=edit
  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden bg-slate-100">
      ...
      {/* Top bar */}
      <div className="shrink-0 flex items-center gap-3 px-5 py-2.5 bg-white border-b border-slate-200 shadow-sm flex-wrap">
        <div className="flex items-center gap-2 mr-1">
          <div className="w-7 h-7 rounded-lg bg-amber-500 flex items-center justify-center shrink-0">
            <Briefcase className="w-4 h-4 text-white" />
          </div>
          <span className="font-black text-slate-800 text-sm whitespace-nowrap">Danh sách Dự án — Nhóm NT-NCC</span>
        </div>

        <div className="flex items-center gap-2 bg-slate-50 border border-slate-200 rounded-xl px-3 py-1.5 min-w-[180px] max-w-xs flex-1">
          ...
          {searchQ && <button onClick={() => setSearchQ('')}><X className="w-3.5 h-3.5 text-slate-400 hover:text-slate-600" /></button>}
        </div>

        <button onClick={handleSync} className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-slate-50 hover:bg-slate-100 border border-slate-200 text-slate-600 text-xs font-bold transition-all">
        ...
        <button onClick={() => setModalKhoi('new')}
          className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-slate-50 hover:bg-slate-100 border border-slate-200 text-slate-700 text-xs font-bold transition-all">
          <FolderPlus className="w-3.5 h-3.5" /> Thêm nhóm dự án mới...
        </button>
      ...
      <div className="flex-1 overflow-x-auto overflow-y-hidden">
        <div className="flex gap-4 px-5 py-4 h-full items-start" style={{ minWidth: 'max-content' }}>
          ...
          <button onClick={() => setModalKhoi('new')}
            className="flex flex-col items-center justify-center gap-2 rounded-2xl border-2 border-dashed border-slate-300 hover:border-blue-400 hover:bg-blue-50/60 transition-all shrink-0 w-[200px] h-[120px] text-slate-400 hover:text-blue-500">
      ...
      {/* Footer */}
      <div className="shrink-0 flex items-center justify-end gap-3 px-6 py-3 bg-white border-t border-slate-200">
        <button onClick={handleDiscard} disabled={!dirty}
          className="px-5 py-2 rounded-xl text-sm font-semibold text-slate-600 hover:bg-slate-100 disabled:opacity-40 disabled:cursor-not-allowed transition-all">
          Hủy bỏ
        </button>
        <button onClick={handleSave} disabled={!dirty}
          className="flex items-center gap-2 px-6 py-2 rounded-xl bg-blue-600 hover:bg-blue-700 disabled:bg-blue-300 disabled:cursor-not-allowed text-white text-sm font-bold transition-all shadow-sm">
      ...
      {/* Toast */}
      {toast && (
        <div className={`fixed bottom-20 right-6 z-[400] flex items-center gap-2.5 px-4 py-3 rounded-xl shadow-2xl border text-sm font-semibold ${
