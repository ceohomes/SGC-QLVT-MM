...
const COLOR_MAP = {
  violet: { header: 'bg-violet-600', border: 'border-violet-200', label: 'text-violet-700' },
  ...
  purple: { header: 'bg-purple-600', border: 'border-purple-200', label: 'text-purple-700' },
}
...
function InputField({ field, value, onChange, error }) {
  const baseInput = `w-full px-3 py-2 border rounded-lg text-sm outline-none transition-all ${
    error ? 'border-rose-400 bg-rose-50 focus:ring-2 focus:ring-rose-200' : 'border-violet-200 focus:border-violet-400 focus:ring-2 focus:ring-violet-100'
  }`
...
export default function EditModal({ isOpen, initialData, onClose, onSave, currentUser }) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 modal-backdrop bg-black/50">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-4xl max-h-[92vh] flex flex-col overflow-hidden border border-violet-200">
        {/* Header */}
        <div className="bg-gradient-to-r from-violet-700 to-purple-700 px-6 py-4 flex items-center justify-between shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-violet-200 text-xs">Điền đầy đủ thông tin, các trường * là bắt buộc</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        {/* Footer */}
        <div className="shrink-0 px-6 py-4 border-t border-violet-100 bg-violet-50 flex items-center justify-end gap-3">
          <button
            onClick={onClose}
          ...
          <button
            onClick={handleSave}
            className="flex items-center gap-2 px-6 h-10 bg-gradient-to-r from-violet-600 to-purple-600 text-white rounded-xl font-bold text-sm shadow-lg shadow-violet-600/30 hover:shadow-violet-600/50 transition-all active:scale-95"
          >
